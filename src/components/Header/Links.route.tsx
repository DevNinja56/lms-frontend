      ...
      {list.map(({ text, Icon, to }, i) => (
        <li
          className={`hover:bg-grayBg hover:text-mainColor group ${
            pathname === to && "bg-grayBg text-mainColor"
          }`}
        ...
        >
          {to !== "#" ? (
            ...
            <button
            className={`flex gap-3 items-center py-2 pl-4 pr-9 w-full ${text ==='Log out'&&'border-t-2 border-gray-100 my-2 mb-0 pt-4'}`}
              onClick={() => handleOnClick(text)}
            >
              <span className="icon">
                <Icon active={pathname === to} />
              </span>
              <span className="text-mainParaColor">{text}</span>
            </button>
          )}
        </li>
      ))}
