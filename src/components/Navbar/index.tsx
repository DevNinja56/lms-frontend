...
const Navbar = () => {
  ...
  return (
    <div className={`h-24 w-full flex justify-between items-center relative ${isHomePage ? 'bg-white' : 'bg-mainColor text-white'} px-24`}>
      <div className={`flex items-center justify-between py-4 px-2 sticky top-0 ${isHomePage ? 'bg-mainColor' : 'bg-[#6b87db] '} rounded-full `}>
      ...
      <div className="flex item-center justify-center gap-3 ">
        ...
        <span className={`text-sm inline-block font-medium  py-2.5 px-5 w-full ${window.location.pathname === ROUTES.POPULAR_COURSES?"border-b":""}`}>
            Popular Courses
          </span>
        </NavLink>
        <NavLink to={ROUTES.BUY_COURSES}>
        <span className={`text-sm inline-block font-medium  py-2.5 px-5 w-full ${window.location.pathname === ROUTES.BUY_COURSES?"border-b":""}`}>
            Buy
          </span>
        </NavLink>
        <NavLink to={ROUTES.FAQ}>
        <span className={`text-sm inline-block font-medium  py-2.5 px-5 w-full ${window.location.pathname === ROUTES.FAQ?"border-b":""}`}>
            FAQs
          </span>
        </NavLink>
...
>
  <path
    ...
    stroke="#495057"
    stroke-width="2"
    style={{ stroke: isHomePage ? '#495057' : 'white' }}
  />
  <path
  ...
  />
        ...
        <NavLink to={ROUTES.SIGN_IN} >
          <span className={`text-sm inline-block font-medium py-2.5 rounded-md px-5 w-full  ${isHomePage ? 'bg-mainColor text-white' : 'bg-white text-mainColor'}`}>
            Portal
          </span>
        </NavLink>
      </div>
