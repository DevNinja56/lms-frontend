...
const SubjectBox = () => {
  ...
  return (
    <div className="whitespace-nowrap">
      <h1 className="font-medium text-[1.2rem] mb-5">Filters</h1>
      ...
      <ul>
        <QuizFilterSelect
          ...
          text={"All"}
          value={"All"}
          defaultChecked={subject?.name === "null"}
          onChange={() => {
            setSubject({ id: "null", name: "null" });
          }}
        />
        {data?.map((sub, i) => {
          return (
            <QuizFilterSelect
              key={"side-filter-subject-box--" + i}
              name="selected-Box"
              text={sub.name}
              value={sub.name}
              defaultChecked={sub.id === subject?.id}
              onChange={() => {
                setSubject(sub);
              }}
            />
