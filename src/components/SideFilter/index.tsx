...
const SideFilter = () => {
  const [isShownFilter, setIsShownFilter] =
    useState(false);
  const {rightFilter} = useUi();

  const AllFilters = {
    [filterContentType.notification]: (
      <Notification />
    ),
    [filterContentType.subjectWeeksDay]: (
      <SubjectWeeksDay />
    ),
    [filterContentType.subscription]: (
      <Subscription />
    ),
    [filterContentType.quizzes]: (
      <QuizzesFilter />
    ),
    [filterContentType.leaderboard]: (
      <LeaderboardFilter />
    ),
    [filterContentType.bookmarksAndNotes]: (
      <BookmarksAndNotes />
    ),
  };
