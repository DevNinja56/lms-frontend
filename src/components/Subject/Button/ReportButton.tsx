...
const ReportButton: React.FC<{
  ...
  const [isReported, setReported] = useState(!!data.userActions[0]?.report);
  const [param] = useSearchParams();
  const type = param.get("type") as days_categoryType;
  const { updateModal } = useUi();

  const handleBookmark = () => {
    !data.userActions[0]?.report &&
      updateModal({
  ...
  return (
    <button
      className="rating grid place-items-center pl-3 md:px-5"
      onClick={handleBookmark}
      disabled={isReported}
    >
      <span className="icon text-xl md:text-2xl text-mainParaColor">
