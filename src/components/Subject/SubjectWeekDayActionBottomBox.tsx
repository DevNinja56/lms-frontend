            ...
            data?.usersActions[0]?.markAsCompleted ? (
              <LinkButton
                to={ROUTES.QUIZZES_DETAILS.replace(
                  ":id",
                  data.usersActions[0].submission.id
                )}
      ...
      <div className="flex text-mainTextColor whitespace-nowrap ">
        <div
          ...
          onClick={() =>
            updateModal({
              ...
              state: {
                rating: data.usersActions?.[0]?.review?.rating ?? 0,
        ...
        >
          ...
          <span className="text-xs/3 capitalize text-mainParaColor">
            {data.usersActions[0]?.review?.rating
              ? `You Rated ${data.usersActions[0]?.review?.rating ?? 0} / 5`
              : `Rate this ${type}`}
          </span>
        </div>
