...
const SubjectWeekHeader = () => {
  ...
  return (
    <>
      <span className="font-bold text-2xl text-mainParaColor block lg:hidden mb-3">
        {week?.name}
      </span>
      <div className="bg-white rounded-[5px] shadow-md shadow-gray-200 py-4 px-3 lg:px-5 xl:px-7 w-full flex justify-between sticky top-0 z-10 mb-9">
        <div className="lg:grid capitalize hidden">
          <span className="font-bold text-2xl text-mainParaColor">
            {week?.name}
          </span>
        </div>
        <div className="flex gap-5 w-full lg:w-auto justify-between lg:justify-start">
          {weeksHeader.map(
            (item: { name: string; length: number; icon: any }, i) => (
              ...
              >
                ...
                <p className="text-[13px] capitalize font-normal text-mainParaColor">
