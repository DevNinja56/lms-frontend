...
const SubjectAndWeeks = () => {
  ...
  const handleBackClick = () => {
    if (navType === navTypes.days_list) {
      navigate(
        ROUTES.SUBJECTS_WEEK.replace(
          ":subject",
          subject.name
        ).replace(":week", week.id! ?? "0")
      );
    } else {
      updateSubjectNav(navTypes.subjects_list);
    }
            ...
            (subject_week, w: number) => (
              <Link
                ...
                className={`block px-[25px] py-4 uppercase font-medium text-mainParaColor ${
                  subject_week.id === week.id &&
                  "bg-grayBg "
                }`}
                key={
                  "weeks-list--a" + w + nanoid()
                }
                onClick={() =>
