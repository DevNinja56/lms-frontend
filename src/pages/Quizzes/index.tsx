...
const Quizzes = () => {
  ...
  useEffect(() => {
    updateFilter({ type: filterContentType.quizzes, state: {} });
  }, []);
      ...
      {CardsData &&
        CardsData?.filter((q) => {
          return quiz_attempted === "all"
            ...
            ? q.usersActions?.[0]?.markAsCompleted
            : !q.usersActions?.[0]?.markAsCompleted;
        })?.map((item, i) => {
          return (
            ...
            <Link
              to={{
                ...
                search: sendParams({
                  type: "quizzes",
                  attempt: !!item.usersActions?.[0]?.markAsCompleted,
                }),
              }}
              key={"quiz-card--" + i}
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 w-20 md:w-auto">
                  ...
                  <div
                    className={`rounded-[3px] flex items-center justify-center text-white font-semi-bold pt-[7px] pb-[6px] px-[14px] text-xs ${
                      item.usersActions?.[0]?.markAsCompleted
                        ? "bg-green-400"
                        : "bg-amber-400"
                    }`}
                  >
